...
Instead of writing one page per project by hand, drop a single directive on an
index MDX page and let the plugin generate a child page per project in a
GitLab group at build time:
...
| `basePath` | string | `"projects"` | Folder name (relative to the index page) the generated child pages are written into |

Generated files land in a **git-ignored** `<basePath>/` folder next to the
index page (e.g. `docs/generate/projects/`) and are regenerated on every
build — never hand-edit them, and never commit them. Generation runs once at
plugin init, before the docs plugin scans the filesystem, so the generated
tree and its `_category_.json` files feed the autogenerated sidebar like any
other doc.
...
On the index page itself, the directive is replaced with a `<GitlabProjectGrid>`
card grid — one card per project, linking to its generated child page. Card
links are built as `<basePath>/<project-slug>` and assume Docusaurus's default
(no-trailing-slash) doc URLs; if your site is configured with
`trailingSlash: true`, adjust routing accordingly.
