...
(`org`, `location`, `bio`, `counts`, `since`) triggers one extra profile lookup per
member at build time (two API requests per member on a cold build; cached on disk
and deduplicated across pages and components).
...
- Bot/service accounts (and deleted users) that have no public profile degrade to
  identity-only cards instead of failing the build.
